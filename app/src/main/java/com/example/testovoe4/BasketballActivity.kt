...
class BasketballActivity : AppCompatActivity() {
    ...
    private fun loadData() {
        GlobalScope.launch(Dispatchers.IO) {
            try {
                val data = URL("http://135.181.248.237/4/basketball.json").readText()
                val gson = Gson()
    ...
    private fun gameOver() {
        ...
        var max = sharedPreferences.getInt("max_basket", 0)
        if (countOfRightAnswers >= max) {
            ...
            editor.putInt("max_basket", max)
            editor.apply()
        }
        val intent = Intent(this, BasketballScoreActivity::class.java)
        intent.putExtra("current_basket", countOfRightAnswers)
