...
class HockeyActivity : AppCompatActivity() {
    ...
    private val countOfQuestions = 15
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Glide.with(this)
            .load("http://135.181.248.237/4/hockey.png")
            .into(imageViewFon2)
    ...
    private fun loadData() {
        GlobalScope.launch(Dispatchers.IO) {
            try {
                val data = URL("http://135.181.248.237/4/hockey.json").readText()
                val gson = Gson()
    ...
    private fun gameOver() {
        ...
        var max = sharedPreferences.getInt("max_hockey", 0)
        if (countOfRightAnswers >= max) {
            ...
            editor.putInt("max_hockey", max)
            editor.apply()
        }
        val intent = Intent(this, HockeyScoreActivity::class.java)
        intent.putExtra("current_hockey", countOfRightAnswers)
